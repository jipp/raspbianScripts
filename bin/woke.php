...
else
{
	...
	// print_r($Client->send("listInterfaces", array()));
	$array = $Client->send("listInterfaces", array());
	foreach( $array as $value ) {
		print "ID: ".$value['ID']."\n";
		$ticks = $value['LASTPACKETRECEIVED'];
		$val = 0;
		exec("date -d @'$ticks'", $val);
		print "  LASTPACKETRECEIVED: ".$val[0]." (".$ticks.")\n";
		$ticks = $value['LASTPACKETSENT'];
		$val = 0;
		exec("date -d @'$ticks'", $val);
		print "  LASTPACKETSENT    : ".$val[0]." (".$ticks.")\n";
	}
